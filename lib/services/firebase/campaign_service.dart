  ...
  Future<List<Campaign>> getCampaignsWithDateRangeTotals({
    ...
    try {
      ...
      for (var campaign in campaigns) {
        ...
        // Debug logging for campaigns with mismatched totals
        final spendDiff = (campaign.totalSpend - totals['totalSpend']).abs();
        if (spendDiff > 1.0) {
          print('⚠️  MISMATCH FOUND:');
          print('   Campaign: ${campaign.campaignName}');
          print('   Date Range: ${campaign.firstAdDate} to ${campaign.lastAdDate}');
          print('   Original totalSpend: \$${campaign.totalSpend.toStringAsFixed(2)}');
          print('   Calculated totalSpend (Oct only): \$${totals['totalSpend'].toStringAsFixed(2)}');
          print('   Difference: \$${spendDiff.toStringAsFixed(2)}');
          print('   Ads in range: ${totals['adsInRange']}');
        }
