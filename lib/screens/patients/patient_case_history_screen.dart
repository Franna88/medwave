  ...
  List<String> _getMissingFieldsForCurrentPage() {
    ...
    switch (_currentPage) {
      ...
      case 1: // Wound Assessment
        if (_woundLocationController.text.trim().isEmpty) {
          missingFields.add('Wound Location');
        }
        if (_woundLengthController.text.trim().isEmpty) {
          missingFields.add('Wound Length');
        }
        if (_woundWidthController.text.trim().isEmpty) {
          missingFields.add('Wound Width');
        }
        if (_woundDepthController.text.trim().isEmpty) {
          missingFields.add('Wound Depth');
        }
        if (_woundDescriptionController.text.trim().isEmpty) {
          missingFields.add('Wound Description');
        }
        break;
