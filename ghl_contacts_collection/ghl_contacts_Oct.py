#!/usr/bin/env python3
"""
GHL Contacts Collection - October 2025
Fetches all GHL contacts for October 2025 and stores them in Firestore collection 'ghl_contacts'
...
def fetch_all_october_contacts():
    """Fetch all contacts for October 2025"""
    print('='*80)
    print('GHL CONTACTS COLLECTION - OCTOBER 2025')
    print('='*80 + '\n')
    ...
    # Filter for October 2025 contacts
    print('='*80)
    print('STEP 2: FILTERING FOR OCTOBER 2025 CONTACTS')
    print('='*80 + '\n')
    ...
    for contact in all_contacts:
        ...
        if date_added:
            # Check if dateAdded is in October 2025
            if date_added >= START_DATE and date_added <= END_DATE:
                october_contacts.append(contact)
    
    print(f'✅ Contacts in October 2025: {len(october_contacts)}')
    print(f'⚠️  Contacts outside October: {len(all_contacts) - len(october_contacts)}\n')
    
    # Verify date range
    if october_contacts:
        print(f'📅 Verifying date range of October contacts...')
        dates = []
        for contact in october_contacts[:5]:
            date_added = contact.get('dateAdded', '')
            if date_added:
                dates.append(date_added[:10])
        
        if dates:
            all_dates = [c.get('dateAdded', '')[:10] for c in october_contacts if c.get('dateAdded')]
    ...
    for contact in october_contacts:
        try:
    ...
    print(f'📊 Summary:')
    print(f'   Total contacts fetched: {len(all_contacts)}')
    print(f'   October 2025 contacts: {len(october_contacts)}')
    ...
    print(f'\n✅ All October 2025 GHL contacts stored in Firestore!\n')


if __name__ == '__main__':
    fetch_all_october_contacts()
