#!/usr/bin/env python3
"""
Facebook Ads Collection - Fetch NEW November 2025 Ads
Fetches only ads that are NOT already in the fb_ads collection
Compares against existing ad IDs in Firestore to avoid duplicates
"""
...
def get_existing_ad_ids():
    """Get all existing ad IDs from fb_ads collection"""
    print('📦 Fetching existing ad IDs from fb_ads collection...\n')
    
    try:
        # Get all documents from fb_ads collection
        docs = db.collection('fb_ads').stream()
        
        existing_ids = set()
        for doc in docs:
            existing_ids.add(doc.id)
        
        print(f'✅ Found {len(existing_ids)} existing ads in collection\n')
        return existing_ids
        
    except Exception as e:
        print(f'❌ Error fetching existing ads: {e}\n')
        return set()
...
def fetch_new_ads_for_november():
    """Fetch only NEW ads that are not already in fb_ads collection"""
    print('='*80)
    print('FACEBOOK ADS COLLECTION - FETCH NEW NOVEMBER 2025 ADS')
    print('='*80 + '\n')
    ...
    # Get existing ad IDs from Firestore
    existing_ad_ids = get_existing_ad_ids()
    
    print('='*80)
    print('STEP 1: FETCHING ALL ADS WITH ACTIVITY IN NOVEMBER')
    print('='*80 + '\n')
    ...
    # Filter out ads that already exist in collection
    print('='*80)
    print('STEP 2: FILTERING NEW ADS')
    print('='*80 + '\n')
    
    new_ads = [ad for ad in all_ads if ad['id'] not in existing_ad_ids]
    
    print(f'📊 Total ads found: {len(all_ads)}')
    print(f'📦 Already in collection: {len(all_ads) - len(new_ads)}')
    print(f'🆕 New ads to fetch: {len(new_ads)}\n')
    
    if len(new_ads) == 0:
        print('✅ No new ads to fetch. Collection is up to date!\n')
        return
    
    # Step 3: Fetch complete data for each NEW ad
    print('='*80)
    print('STEP 3: FETCHING COMPLETE DATA FOR NEW ADS')
    print('='*80 + '\n')
    ...
    for i, ad in enumerate(new_ads, 1):
        ...
        print(f'{i}/{len(new_ads)}. {ad_name[:60]}')
        print(f'   Ad ID: {ad_id}')
    ...
    print('='*80)
    print('STEP 4: STORING NEW ADS IN FIRESTORE COLLECTION "fb_ads"')
    print('='*80 + '\n')
    ...
    print(f'📊 Summary:')
    print(f'   Total ads in November: {len(all_ads)}')
    print(f'   Previously in collection: {len(all_ads) - len(new_ads)}')
    print(f'   New ads found: {len(new_ads)}')
    print(f'   New ads with complete data: {len(ads_with_complete_data)}')
    ...
    print(f'\n✅ All NEW November 2025 ads stored in Firestore!\n')
